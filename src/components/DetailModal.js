export default class DetailModal {
  ...
  render() {
    ...
    const {imperial, metric} = this.data.breeds[0] ?
      this.data.breeds[0] : {imperial: '정보없음', metric: '정보없음'};

    /* wrapper 위에서 화면 전체를 감싸는 부분 */
    ...
    catWeight.textContent = `${imperial} (imperial) / ${metric} (metric)`
